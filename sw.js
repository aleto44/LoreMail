...
}, b = (a) => [f.prefix, a, f.suffix].filter((e) => e && e.length > 0).join("-"), x = (a) => {
...
async function S(a, e, t, s) {
  ...
  if (e.url === n)
    return a.match(e, s);
  const r = Object.assign(Object.assign({}, s), { ignoreSearch: !0 }), i = await a.keys(e, r);
  for (const c of i) {
    const o = P(c.url, t);
    if (n === o)
      return a.match(c, s);
...
class B {
  ...
  async cachePut(e, t) {
    ...
    if (!r)
      return !1;
    const { cacheName: i, matchOptions: c } = this._strategy, o = await self.caches.open(i), h = this.hasCallback("cacheDidUpdate"), g = h ? await S(
  ...
  *iterateCallbacks(e) {
  ...
  async doneWaiting() {
    for (; this._extendLifetimePromises.length; ) {
      const e = this._extendLifetimePromises.splice(0), s = (await Promise.allSettled(e)).find((n) => n.status === "rejected");
      if (s)
        throw s.reason;
    }
  }
...
class $ {
  ...
  handleAll(e) {
    ...
    const t = e.event, s = typeof e.request == "string" ? new Request(e.request) : e.request, n = "params" in e ? e.params : void 0, r = new B(this, { event: t, request: s, params: n }), i = this._getResponse(r, s, t), c = this._awaitComplete(i, r, s, t);
    return [i, c];
  }
    ...
    try {
      ...
      if (r instanceof Error) {
        for (const i of e.iterateCallbacks("handlerDidError"))
          if (n = await i({ error: r, event: s, request: t }), n)
            break;
      }
      if (!n)
        throw r;
...
class G {
  ...
  addToCacheList(e) {
    ...
    for (const s of e) {
      ...
      const { cacheKey: n, url: r } = M(s), i = typeof s != "string" && s.revision ? "reload" : "default";
...
function J(a, e, t) {
  let s;
  if (typeof a == "string") {
    const r = new URL(a, location.href), i = ({ url: c }) => c.href === r.href;
    s = new w(i, e, t);
